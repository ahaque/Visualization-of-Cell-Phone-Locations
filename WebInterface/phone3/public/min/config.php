...
$min_documentRoot = substr( dirname(  __FILE__ ), 0, -11 );

$min_cachePath = $min_documentRoot . '/cache/tmp';
...
/* Tiny hack for IN_DEV */
if ( isset( $_GET['f'] ) )
{
	if ( strstr( $_GET['f'], '/master_css/' ) )
	{
		/* Turn off URL rewriting and junk folder name so ../../ is correct */
		$min_serveOptions['rewriteCssUris']   = false;
		$min_serveOptions['minifierOptions']['text/css']['prependRelativePath'] = 'public/';
	}
	else if ( strstr( $_GET['f'], '/skin_cp/' ) )
	{
		/* Fetch ACP dir name */
		list( $acpDir, $junk )  = explode( "/", $_GET['f'] );
		list( $folders, $junk ) = explode( "/public", $_SERVER['SCRIPT_NAME'] );
...
if( isset($_GET['charset']) AND $_GET['charset'] )
{
	$min_serveOptions['contentTypeCharset']	= preg_replace( "/[^a-zA-Z0-9\-_]/", '', $_GET['charset'] );
}
