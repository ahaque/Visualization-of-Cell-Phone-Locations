...
<ol id='member_alpha' class='tab_bar no_title'>
	" . ((! in_array( $this->settings['search_method'], array( 'traditional', 'sphinx' ) )) ? ("" . ((isset( $this->request['search_filter_app'] ) && $this->request['search_filter_app']['all'] != 1) ? ("
		...
		")) . "") : ("")) . "
	".$this->__f__67ca02bfdf542496ffe49139edce8705($results,$pagination,$total)."</ol>
" . (($total) ? ("
		<div id='search_results'>
		<ol>
			".$this->__f__9eea14363387809abff8b8873b05c2ba($results,$pagination,$total)."		</ol>
	</div>
	{$pagination}
") : ("
...
function __f__67ca02bfdf542496ffe49139edce8705($results, $pagination, $total)
{
	$_ips___x_retval = '';
...
function __f__9eea14363387809abff8b8873b05c2ba($results, $pagination, $total)
{
	$_ips___x_retval = '';
...
<ol id='member_alpha' class='tab_bar no_title'>
	" . ((! in_array( $this->settings['search_method'], array( 'traditional', 'sphinx' ) )) ? ("" . ((isset( $this->request['search_filter_app'] ) && $this->request['search_filter_app']['all'] != 1) ? ("
		...
		")) . "") : ("")) . "
	".$this->__f__6d76cd47111d727807a80bce5feada63($results,$pagination,$total)."</ol>
" . (($total) ? ("
		<div id='search_results'>
		<ol>
			".$this->__f__dcbbd33b8253ea93d185426526855c89($results,$pagination,$total)."		</ol>
	</div>
	{$pagination}
") : ("
...
function __f__6d76cd47111d727807a80bce5feada63($results, $pagination, $total)
{
	$_ips___x_retval = '';
	foreach( $this->registry->getApplications() as $app )
	{
...
function __f__dcbbd33b8253ea93d185426526855c89($results, $pagination, $total)
{
	$_ips___x_retval = '';
	foreach( $results as $result )
	{
...
") : ("")) . "
	...
	<ol id='member_alpha' class='tab_bar'>
		...
		")) . "
		".$this->__f__22eb42745ee94a718d6ce0fbb2adae95($results,$pagination,$total,$showing,$search_term,$url_string,$current_key,$removed_search_terms)."	</ol>
	<div class='tab_filters'>
		<ul class='right'>
			...
			") : ("
				<li><a href='" . $this->registry->getClass('output')->formatUrl( $this->registry->getClass('output')->buildUrl( "{$url_string}&amp;search_sort_by=relevence&amp;search_filter_app[{$current_key}]=1", 'public','' ), "", "" ) . "'>{$this->lang->words['sort_by_relevance']}</a></li>
			")) . "
	
			" . (($this->request['search_sort_by'] == 'date') ? ("
				<li class='active'>{$this->lang->words['sort_by_date']}</li>
			") : ("
				<li><a href='" . $this->registry->getClass('output')->formatUrl( $this->registry->getClass('output')->buildUrl( "{$url_string}&amp;search_sort_by=date&amp;search_filter_app[{$current_key}]=1", 'public','' ), "", "" ) . "'>{$this->lang->words['sort_by_date']}</a></li>
			")) . "
		</ul>
	</div>
...
" . (($total) ? ("
		<div id='search_results'>
		<ol>
			".$this->__f__fd549b30c15735947d2d0bc0c4e7702f($results,$pagination,$total,$showing,$search_term,$url_string,$current_key,$removed_search_terms)."		</ol>
	</div>
	<br />
	<div class='topic_controls'>
		{$pagination}
	</div>
") : ("<p class='no_messages'>{$this->lang->words['no_results_found']}" . (($search_term) ? (" {$this->lang->words['no_results_found_for']} '{$search_term}'") : ("")) . ".</div>")) . "";
return $IPBHTML;
}


function __f__22eb42745ee94a718d6ce0fbb2adae95($results, $pagination, $total, $showing, $search_term, $url_string, $current_key, $removed_search_terms=array())
{
	$_ips___x_retval = '';
	foreach( $this->registry->getApplications() as $app )
	{
...
function __f__fd549b30c15735947d2d0bc0c4e7702f($results, $pagination, $total, $showing, $search_term, $url_string, $current_key, $removed_search_terms=array())
{
	$_ips___x_retval = '';
	foreach( $results as $result )
	{
...
$IPBHTML .= "<div class='result_info'>
	...
	" . (($r['_forum_trail']) ? ("
		<span class='desc breadcrumb'>
			".$this->__f__dabec9bdcebbdcb30aada4b8fc275400($r,$indent,$resultAsTitle)."		</span>
	") : ("")) . "
</div>
<div class='result_details desc'>
	<ul>
...
function __f__dabec9bdcebbdcb30aada4b8fc275400($r, $indent, $resultAsTitle=false)
{
	$_ips___x_retval = '';
...
$IPBHTML .= "<tr class='" .  IPSLib::next( $this->registry->templateStriping["forumTable"] ) . "' id='trow_{$data['tid']}'>
	...
	<td>
		...
		" . ((isset( $data['pages'] ) AND is_array( $data['pages'] ) AND count( $data['pages'] )) ? ("
			...
			".$this->__f__16276d4bfe6781d65ecb19cd5432c068($data,$childEntry,$resultAsTitle)."			</ul>
		") : ("")) . "
	...
	<td class='altrow'>
		".$this->__f__f5b02f8a16ab4c013efbd4d26744aeb9($data,$childEntry,$resultAsTitle)."	</td>
...
function __f__16276d4bfe6781d65ecb19cd5432c068($data, $childEntry=0, $resultAsTitle=false)
{
	$_ips___x_retval = '';
...
function __f__f5b02f8a16ab4c013efbd4d26744aeb9($data, $childEntry=0, $resultAsTitle=false)
{
	$_ips___x_retval = '';
...
" . (($total) ? ("<p class='message'>
		{$this->lang->words['user_posts_have_been']} <em><strong>{$total}</strong></em> " . (($this->request['view_by_title'] && $this->request['search_app'] == 'forums') ? ("{$this->lang->words['user_posts_title_topics']}") : ("{$this->lang->words['user_posts_title_posts']}")) . " {$this->lang->words['user_posts_title_member']}
	</p>
	<br />") : ("")) . "
<div class='topic_controls'>
	{$pagination}
...
<ol id='member_alpha' class='tab_bar no_title'>
	" . ((! in_array( $this->settings['search_method'], array( 'traditional', 'sphinx' ) )) ? ("" . ((isset( $this->request['search_filter_app'] ) && $this->request['search_filter_app']['all'] != 1) ? ("
		...
		")) . "") : ("")) . "
	".$this->__f__47beb649a03e6ac0faaad715cb98b843($results,$pagination,$total,$member)."</ol>
" . (($total) ? ("
		<div id='search_results'>
		<ol>
			".$this->__f__cd97c41ab437d5c9776faf569eda4632($results,$pagination,$total,$member)."		</ol>
	</div>
") : ("
...
function __f__cd97c41ab437d5c9776faf569eda4632($results, $pagination, $total, $member)
{
	$_ips___x_retval = '';
	foreach( $results as $result )
	{
