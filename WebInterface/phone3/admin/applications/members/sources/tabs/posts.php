...
class profile_posts extends profile_plugin_parent
{
	...
	public function return_html_block( $member=array() ) 
	{
		...
		$this->DB->build( array( 
									'select'		=> 'p.*',
									'from'		=> array( 'posts' => 'p' ),
									'where'		=> "p.queued=0 AND t.approved=1 AND p.author_id={$member['member_id']} AND p.new_topic=0 AND {$this->registry->permissions->buildPermQuery('i')} AND {$this->registry->permissions->buildPermQuery('i', 'perm_2')} {$trash_can_remove} AND (f.password='' OR f.password" . $this->DB->buildIsNull() . ")",
