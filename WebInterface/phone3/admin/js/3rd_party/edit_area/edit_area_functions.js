	...
	EditArea.prototype.toggle_full_screen= function(to){
		...
		{	// toogle on fullscreen		
			...
			t.fullscreen['old_fHeight']	    = p.get_css_property(frame, "height");
			t.fullscreen['old_fWidth']		= p.get_css_property(frame, "width");
		...
		//	body.appendChild(frame);
			...
			if ( t.isSafari )
			{
				p.document.getElementsByTagName('body')[0].scrollTo();
			}
		...
		{	// toogle off fullscreen
			...
			frame.style.height	= parseInt( t.fullscreen['old_fHeight'] ) + 'px';
			frame.style.width	= parseInt( t.fullscreen['old_fWidth'] ) + 'px';
